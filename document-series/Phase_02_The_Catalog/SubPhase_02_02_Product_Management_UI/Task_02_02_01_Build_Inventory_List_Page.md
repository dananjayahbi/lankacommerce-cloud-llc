...
Create `src/app/dashboard/[tenantSlug]/inventory/page.tsx` as a server component that reads the `tenantSlug` from the route params and passes it to the client-side `InventoryListClient` component. The server component performs an initial product count fetch server-side for the page title subtitle by calling the Django endpoint `GET /api/catalog/products/?count_only=true` using the standard `fetch()` function, keeping the count instantly available on first render without a loading flash.

The page component extracts the current user's auth context using the `getAuthFromCookies()` server utility from Phase 01. This utility reads the `access_token` httpOnly cookie via `cookies()` from `next/headers`, decodes the JWT claims, and returns a permissions object. If the access token is absent or expired, redirect immediately to the login page. Pass the resolved permissions object to the client component so permission gates can render without a round-trip.

### Step 2: Build the Page Header Row

The page header is a horizontal flex row with three elements. On the left: the heading "Inventory" in Inter H1 with navy colour, and a subtitle line in Inter that reads "X products" where X is the total count returned from the initial fetch — this updates reactively when the TanStack Query data resolves. On the right: an "Add Product" button using the navy background with surface text, which links to `/dashboard/[tenantSlug]/inventory/new`. This button is conditionally rendered — it only shows when the current user has the `product:create` permission. An "Import CSV" secondary button with a border outline sits next to Add Product and links to the import route.

### Step 3: Build the useProducts Hook

Create `src/hooks/useProducts.ts`. This hook accepts a filters object containing `search`, `categoryIds`, `brandIds`, `genders`, `statuses`, `page`, and `limit` fields. It constructs a `URLSearchParams` string from these values and calls the Django endpoint `GET /api/catalog/products/` with that query string, passing the JWT as a `Bearer` token in the `Authorization` header. The JWT is read from the auth context provided by the `useAuth()` hook. Configuration: `staleTime` of 30 000 milliseconds, `keepPreviousData` enabled so pagination transitions do not flash to a loading state. The hook returns the products array, total count, page info, and status flags.

### Step 4: Build the InventoryTable Component

Create `src/components/inventory/InventoryTable.tsx`. The table uses a border-coloured header row with Inter font labels. Each data row has a surface background with an orange hover highlight applied via Tailwind on the `tr` element. Columns are as follows:

| Column         | Description                                                                                                                             |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| Checkbox       | Selects the row for bulk operations; header checkbox selects all visible rows                                                           |
| Product Name   | Displays a small 40 × 40 px thumbnail from the first variant image, or a clothing icon placeholder in text-muted colour if no image exists. Product name in Inter semibold next to it |
| Category       | Plain text category name, or an em-dash if uncategorised                                                                                |
| Brand          | Plain text brand name, or an em-dash if no brand                                                                                        |
| Gender         | Rendered as a small pill badge with border background and navy text                                                                     |
...
The empty state — shown when the query returns zero results — displays a friendly illustration placeholder (a coat-hanger SVG icon in text-muted colour), a heading "No products yet" in Inter, a subtitle "Start building your catalog", and the navy "Add Product" button. If the empty state is due to active filters returning no results, replace the heading with "No products match your filters" and the action with a "Clear filters" text link.
...
| ------------- | ---------- | ------------- | ---------------------------------------------------------------------------------------------------- |
| Active        | #22C55E    | surface       | Product is active and all variants above threshold                                                   |
| Low Stock     | #F59E0B    | surface       | Product is active and at least one variant is at or below its `lowStockThreshold`                    |
| Out of Stock  | #EF4444    | surface       | Product is active and at least one variant has `stockQuantity` of zero                               |
| Archived      | text-muted | navy          | Product status is `ARCHIVED`                                                                         |
...
- [ ] All columns display correct data fetched from the `/api/catalog/products/` endpoint
