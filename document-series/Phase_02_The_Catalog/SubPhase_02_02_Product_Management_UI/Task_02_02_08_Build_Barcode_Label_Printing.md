...
On the Product Detail page Variants tab, add a "Print Labels" secondary button (border outline) in the toolbar above the variants table. When one or more variant checkboxes are selected, this button becomes active and shows the count: "Print Labels (3)". Clicking it with no checkboxes selected shows a toast "Select at least one variant to print labels."
...
Create `src/components/inventory/BarcodeLabelDialog.tsx`. This is a wide ShadCN Dialog (minimum 700 px width) with the title "Print Barcode Labels" in Inter semibold. The Dialog body is split into two sections: a left panel showing the label preview grid and a right panel showing the print settings.
...
- Brand name: Inter 8 px, navy colour, left-aligned, top of the label
- A thin 1 px text-muted horizontal rule below the brand name
- Product Name: Inter 11 px, navy, left-aligned, wrapping to two lines maximum
- SKU: JetBrains Mono 9 px, navy, left-aligned. The prefix "SKU:" is in text-muted colour
- Size and Colour: Inter 8 px, text-muted colour, in the format "Size: M · Colour: Midnight Blue"
- Barcode image: rendered as an inline SVG using the `react-barcode` package (or a direct SVG-based Code128 rendering utility). The barcode string comes from the variant's `barcode` field. If the `barcode` field is empty, fall back to encoding the SKU as the barcode value. The barcode is centred horizontally with 4 px margins on either side. Auto-generated barcodes in LankaCommerce use the "LKC" prefix (15 chars total: "LKC" + 12 alphanumeric)
- A 4 px gap below the barcode
- Retail Price: Inter bold 14 px, navy, right-aligned, formatted as "Rs. 1,250.00". This is the largest text element on the label for quick visual scanning
- A low-stock dot: if the variant's `stockQuantity` is at or below its `lowStockThreshold`, render a small 8 px filled circle in warning colour (#F59E0B) to the left of the retail price on the same baseline row. This gives pickers visual awareness without cluttering the label for normal-stock items

The entire label component is bounded by a 4 cm × 6 cm box in print mode. On screen (preview), it is bounded by a 160 × 240 px container with a navy 1 px border and a background-colour background.
...
The Dialog footer contains: "Cancel" (text-muted ghost) and "Print Labels" (navy fill). Clicking "Print Labels" does the following:
...
If a variant has no `barcode` value and no `sku` value, the `BarcodeLabel` component should gracefully render a placeholder message in text-muted italic: "No barcode available" in place of the barcode SVG. This prevents a rendering crash when a variant was imported without these values.
...
- [ ] `BarcodeLabel` component renders all content sections (brand, rule, name, SKU, size/colour, barcode, price)
- [ ] Barcode SVG encodes the variant's `barcode` value; falls back to SKU if barcode is empty
- [ ] Low-stock warning dot (#F59E0B) appears when `stockQuantity` is at or below `lowStockThreshold`
