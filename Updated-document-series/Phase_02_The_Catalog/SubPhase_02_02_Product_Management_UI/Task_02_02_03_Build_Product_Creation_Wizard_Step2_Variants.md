...
| Retail Price        | Numeric input           | Rs. prefix label, right-aligned; turns an orange border if the value is less than the cost price in the same row                 |
...
The table renders inside a horizontally scrollable container on smaller screens. The header row is border-coloured with Inter semibold labels. Data rows use surface background with orange hover.
...
The bottom navigation row contains "← Back" (text-muted outline) which calls `goToStep(1)` without clearing step 1 data, and "Next: Review →" (navy primary) which triggers the validation and advances to step 3 on success.
...
- [ ] A retail price value below cost price on a row turns the retail price input border orange
...
- Do not attempt to validate SKU uniqueness against the database in this step — the uniqueness check happens server-side during the `POST /api/catalog/products/` request in Step 3
