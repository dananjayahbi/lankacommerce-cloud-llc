...
Both page server components (`src/app/dashboard/[tenantSlug]/inventory/categories/page.tsx` and `src/app/dashboard/[tenantSlug]/inventory/brands/page.tsx`) must check the `inventory:manage` permission using `getAuthFromCookies()` before rendering. This utility decodes the JWT from the `access_token` httpOnly cookie and returns the `permissions` array. If the `inventory:manage` permission is absent, redirect to `/dashboard/[tenantSlug]/inventory` with a 302. This prevents CASHIER roles from accessing these management screens even by typing the URL directly.
...
The page header follows the standard inventory page header pattern: Inter H1 "Categories" in navy, a subtitle showing the total category count in Inter small, and a "New Category" button with navy fill in the top-right.

### Step 3: Build the CategoryTree Component

Create `src/components/categories/CategoryTree.tsx`. This component fetches categories from the Django endpoint `GET /api/catalog/categories/` using the `useCategories` hook, passing the JWT as a `Bearer` token. The tree supports exactly two levels: top-level parent categories and their direct children. Grandchild categories are not supported in the data model.
...
- A product count badge in text-muted small text showing how many products belong to that category (direct assignment, not counting children)
...
When `isEditing` is true for a category row, the name text is replaced by a controlled text input pre-filled with the current name. Pressing Enter or clicking a checkmark confirm icon saves the change via `PATCH /api/catalog/categories/{id}/`. Pressing Escape cancels and restores the original name. The focus management must return focus to the confirm button after the save completes.

Clicking the disclosure triangle expands or collapses the children rows with a smooth CSS height transition. Children rows are indented by 24 px and shown in Inter small with a slightly lighter navy text colour.

### Step 4: Build the Inline Category Form

Create `src/components/categories/InlineCategoryForm.tsx`. Clicking "New Category" in the page header reveals this form as an inline card directly below the header rather than opening a modal. The form contains: a "Parent Category" select (optional — defaults to no parent for a top-level category), a "Category Name" text input, and two buttons: "Add Category" (navy fill) and "Cancel" (text-muted link text). Clicking Cancel hides the form by setting a local `showForm` state to false. Clicking Add Category validates the name (minimum 2 characters, maximum 50 characters) and calls `POST /api/catalog/categories/`. On success, invalidate the categories query and hide the form.

### Step 5: Build the Delete Guard Behaviour

Before calling `DELETE /api/catalog/categories/{id}/`, the client checks whether the category's product count is greater than zero. If it is, instead of making the API call, show a ShadCN Sonner toast with the message "Cannot delete — [N] products are assigned to this category. Reassign them first." The delete icon is hidden in the tree if `productCount` is greater than zero, but the Django API must also enforce this guard server-side by returning a 409 Conflict status if products reference the category at the time of deletion.
...
Create `src/components/brands/BrandsTable.tsx`. This table has four columns: Brand Name (Inter semibold), Logo (a 40 × 40 px thumbnail image, or a placeholder rectangle in text-muted colour if no logo is set), Product Count (integer), and Actions (Edit and Delete icon buttons). The table header is border-coloured. Each row has a surface background with orange hover.

Clicking Edit on a brand row opens the `BrandEditSheet`.

### Step 7: Build the BrandEditSheet Component

Create `src/components/brands/BrandEditSheet.tsx`. This right-side Sheet is used for both creating a new brand and editing an existing one. The Sheet header shows "Edit Brand" or "New Brand" depending on the context, in Inter semibold.

Form fields: Brand Name (required text input), Description (optional Textarea), and a Logo image uploader using the same `ProductImageUpload` component from Task_02_02_11, but limited to a single image. Clicking Save calls `PATCH /api/catalog/brands/{id}/` for edits or `POST /api/catalog/brands/` for new brand creation, passing the JWT as a `Bearer` token, then invalidates the brands query.
...
- All API calls in this sub-task must include the JWT `Bearer` token in the `Authorization` header; the `useCategories` and `useBrands` hooks should handle this automatically using the `useAuth()` hook
