...
Build the CSV Import interface that allows OWNER and MANAGER roles to bulk-load products and variants into the tenant's catalog from a spreadsheet. The interface guides users through three steps: uploading the file, mapping CSV columns to LankaCommerce fields, and previewing the parsed rows before confirming the import. The result is a summary of how many records were created, warned, or skipped.
...
Create `src/app/dashboard/[tenantSlug]/inventory/import/page.tsx` as a client component. Guard access with the `product:create` permission read from the JWT via the `useAuth()` hook — users without it are redirected to `/inventory`. The page has the standard background colour and displays a surface card centred with a max-width of 860 px. Page header: Inter H1 "Import Products" in navy, a breadcrumb "Inventory → Import Products", and a link "Download CSV template" in orange that triggers download from the Django endpoint `GET /api/catalog/csv-template/`.
...
Create `src/components/csv/CsvUploadZone.tsx`. This component renders a large drag-and-drop area occupying the full width of the content card and standing 200 px tall. The zone has a border dashed border (border-dashed, 2 px), a background colour, and centred content: an upload icon, the text "Drag your CSV file here, or click to browse" in Inter, and a small note "Supports .csv files up to 5 MB" in text-muted small.

When a file is dragged over the zone, the background transitions to an orange 10% tinted background and the border becomes solid orange. This visual cue is implemented with the HTML drag events: `onDragEnter`, `onDragOver`, and `onDragLeave`.
...
The expected CSV format is documented here for reference. Each row in the CSV represents one product variant. Multiple rows with the same Product Name are grouped into a single product with multiple variants during the import server-side processing on the Django backend.
...
Create `src/components/csv/ColumnMappingTable.tsx`. This component receives the parsed CSV data (array of row objects) and the detected headers array. It displays a mapping table with one row per LankaCommerce expected field.

Each row in the mapping table has three columns:

- LankaCommerce Field: the expected field name in Inter semibold, with a "(Required)" suffix in danger red for required fields
...
- Preview: shows the first three non-empty values from the auto-detected (or user-selected) column, displayed in Inter small text-muted italic separated by commas
...
- Warning: the row is importable but some optional data is missing or unrecognised — for example the category name does not match any existing category (a new category will be created by the Django backend), or a non-critical field is malformed
...
Below the summary bar, two action options: "Skip [Z] errors and import [X + Y] rows" (navy fill) and "Go back to fix errors" (text-muted outline). Choosing to skip is the default happy path — the valid and warning rows proceed to import.

When "Skip errors and import" is confirmed, the component calls `POST /api/catalog/products/import/` on the Django backend with the validated JSON payload (only the valid and warning rows serialised as an array of product/variant objects). The JWT is passed as a `Bearer` token in the `Authorization` header. While the import is running, show a ShadCN Progress bar component updating as the server streams progress (or as a simulated progress if the endpoint does not stream). On completion, show a summary toast: "[created] products created, [variants] variants added." and redirect to `/dashboard/[tenantSlug]/inventory`.

---

## Expected Output

Navigating to `/inventory/import` shows the upload zone. Dropping a well-formed CSV file parses it and shows the row count and headers. Advancing to step 2 shows the column mapping table with auto-detected matches. Advancing to step 3 shows the preview table with status badges per row. Importing valid rows creates the products and variants on the Django backend and redirects to the inventory list.
...
- [ ] Selecting "Skip errors" posts only the passing rows to `POST /api/catalog/products/import/`
...
- The `POST /api/catalog/products/import/` Django endpoint must validate the incoming payload server-side with DRF serializers or Pydantic, even though the client already validated it — never trust client-side validation alone
- The Django import view runs inside `transaction.atomic()`. If the transaction fails partway through (for example a duplicate SKU constraint violation), the entire import rolls back and the API returns a 409 with a descriptive message
...
- The CSV template download from `GET /api/catalog/csv-template/` should return a minimal example CSV with two rows covering a simple product (a shirt with a Small and a Large variant) so users immediately understand the expected format
