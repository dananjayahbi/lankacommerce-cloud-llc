...
class TestDrawerView(APIView):
    authentication_classes = [JWTAuthentication]
    permission_classes = [IsAuthenticated]

    def post(self, request):
        if not IsManagerOrAbove().has_permission(request, self):
            return _error("FORBIDDEN", "Only managers and owners may test the drawer.", 403)
        ...
        if not config or not config.get("printer_type"):
            return _error(
                "MISCONFIGURED",
                "Printer is not configured. The drawer is connected via the printer.",
                400,
            )

        try:
            from apps.hardware.services.cash_drawer_service import test_drawer
            test_drawer(config)
        except ImportError:
            return _error("PRINTER_ERROR", "The python-escpos library is not installed.", 500)
            ...
            return _error("PRINTER_ERROR", "An unexpected drawer error occurred.", 500)

        response = _ok({"message": "Drawer test successful."})
