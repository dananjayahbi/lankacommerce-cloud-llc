...
export default function StockTakesPage() {
  ...
  const { mutate: startSession, isPending: starting } = useMutation({
    mutationFn: async () => {
      ...
      if (res.status === 409) {
        ...
        if (err?.error?.code === "SESSION_ALREADY_IN_PROGRESS") {
          router.push(`/store/stock-control/stock-takes/${err.error.existing_session_id}`);
          return null;
        }
      }
    ...
    onSuccess: (data) => {
      ...
      router.push(`/store/stock-control/stock-takes/${data.data.id}`);
    },
    onError: () => {
                ...
                : sessions.map((s) => (
                  <tr key={s.id} className="border-b border-[#E2E8F0] last:border-0 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      {s.status === "IN_PROGRESS" && (
                        <Link
                          href={`/store/stock-control/stock-takes/${s.id}`}
                      ...
                      {s.status === "PENDING_APPROVAL" && (
                        <Link
                          href={`/store/stock-control/stock-takes/${s.id}/review`}
